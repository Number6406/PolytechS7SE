...
public class Logger {
    ...
    public void productionLogger(_Producteur p, Message m) {
        System.out.format("[LOG:%3d|%s] <%s:%2d> production d'un message : (%s)%n", currentId(), dateFormat.format(LocalDateTime.now()), PROD_IDENTIFIER, p.identification(), m.toString());
    }
    
    public void consommationLogger(_Consommateur c, Message m) {
        System.out.format("[LOG:%3d|%s] <%s:%2d> consommation d'un message : (%s)%n", currentId(), dateFormat.format(LocalDateTime.now()), CONS_IDENTIFIER, c.identification(), m.toString());
    }
